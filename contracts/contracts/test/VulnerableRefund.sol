...
///
/// Deliberately vulnerable refund: the external call happens BEFORE the
/// caller's stake is zeroed, and there is no reentrancy guard. It exists so the
/// test suite can prove that the attack technique actually works, rather than
/// asserting that a guard "would" help.
///
/// See `Voting.t.sol::test_Reentrancy_*` for the three-way comparison against
/// `CEIOnlyRefund` and the production `Voting` contract.
contract VulnerableRefund {
    ...
    /// @dev VULNERABLE: interaction before effect, and no guard.
