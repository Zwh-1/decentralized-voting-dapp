...
**"读不到"绝不能显示成"没问题"。** 规则比对有三个结果：`unchanged` / `changed` / `unknown`。只要任意一侧读不到（读取失败、空值、零哈希），就必须是 `unknown`——把读取失败渲染成"规则完好"正是这个功能要消除的那种虚假保证。同理，索引不可用时活动记录返回 **404 并显示「这个部署没有可用的索引」**，而不是显示成"还没有任何活动"。

见 [ADR-0029](docs/aegis/adr/ADR-0029-rules-commitment-makes-edits-visible.md)。
