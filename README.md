...
| 部署脚本顺序性质（桩化）           | **91 项全过 / 0 失败**，退出码 0。断言的是外部看不见的性质：目标槽健康门通过**之前**没有 `nginx -s reload`；成功部署**之后**没有 `stop` 旧槽；migrate 只在起槽**之前**跑一次；**nginx 从未启动过的主机也能完成第一次渲染**（否则第一次发布必然死在切流量那一步）；自签证书下 `INSECURE_TLS=1` 确实传给了 curl、并写进探测摘要 | `bash ops/deploy/selftest.sh`                                       |
| 上面 91 项**会失败**（负向对照）   | **7 项全过**：干净副本通过；4 组注入故障各被抓住——把流量切换提到健康门之前（2 条断言抓到）、成功部署后停掉旧槽（**1 条**）、把 migrate 挪到起槽之后（**7 条**）、健康门失败仍记录成功（**6 条**）；还原后再次通过                                                                                                             | `python ops/deploy/selftest-controls.py`                            |
...
>
> - **已完成并有实测支撑的**：`docker compose config --quiet` 在基础与生产两种 profile 下**退出码均为 0**（即这些文件确实能被真实的 compose CLI 解析，而不是"看起来像 YAML"）；`bash -n` 覆盖 `ops/` 下 **13 个** shell 脚本、**0 失败**；部署脚本桩化自测 **91 项全过**，其负向对照 **7 项全过**；健康门与探测循环打过**真实 HTTP 服务器**。详见[部署与运维](#部署与运维)与[基线记录](docs/aegis/baseline/2026-09-22-containerization-and-delivery.md)。
...
| 部署脚本的顺序性质（桩化）                                           | **已实测**：`ops/deploy/selftest.sh` **91 项全过**                                                                                                                                                                                                                                               |
| 上面那 91 项**会失败**                                               | **已实测**：`ops/deploy/selftest-controls.py` **7 项全过**——4 组注入故障各被抓住，改回后再次通过                                                                                                                                                                                                 |
...
| **`nginx -t` 与 `nginx -s reload`**                                  | **未实测**：从未真正执行；graceful 行为未观察。渲染后的校验现在跑在**一次性容器**里（`compose run --rm --no-deps nginx nginx -t`），这是为了让"nginx 从未启动过的主机"也能完成第一次渲染——代价是校验不再针对运行中的那个容器，两点都未在真实引擎上跑过                                           |
| **两个槽同时运行**                                                   | **未实测**：从未同时启动过                                                                                                                                                                                                                                                                       |
