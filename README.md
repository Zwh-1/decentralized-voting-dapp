...
| 指标                               | 实测结果                                                                                                                                                                                                                                                                                                                      | 复现命令                                                            |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| shell 脚本语法                     | `ops/` 下 **13 个** shell 脚本（12 个 `.sh` + `ops/server/ufw.rules`，与 CI 的扫描口径一致）全部通过 `bash -n`，**0 失败**                                                                                                                                                                                                    | `find ops \( -name '*.sh' -o -name '*.rules' \) -exec bash -n {} +` |
| 部署脚本顺序性质（桩化）           | **93 项全过 / 0 失败**，退出码 0。断言的是外部看不见的性质：目标槽健康门通过**之前**没有 `nginx -s reload`；成功部署**之后**没有 `stop` 旧槽；migrate 只在起槽**之前**跑一次；**nginx 从未启动过的主机也能完成第一次渲染**（否则第一次发布必然死在切流量那一步）；自签证书下 `INSECURE_TLS=1` 确实传给了 curl、并写进探测摘要 | `bash ops/deploy/selftest.sh`                                       |
| 上面 93 项**会失败**（负向对照）   | **7 项全过**：干净副本通过；4 组注入故障各被抓住——把流量切换提到健康门之前（2 条断言抓到）、成功部署后停掉旧槽（**1 条**）、把 migrate 挪到起槽之后（**7 条**）、健康门失败仍记录成功（**6 条**）；还原后再次通过                                                                                                             | `python ops/deploy/selftest-controls.py`                            |
...
| 零停机探测**仪器**本身             | 全成功 6s → `{"total":5,"succeeded":5,"failed":0,"maxConsecutiveFailures":0}` 退出 **0**；全失败 → `{"total":3,"failed":3,"maxConsecutiveFailures":3,"firstFailureAtSeconds":0}` 退出 **1**（`INSECURE_TLS=1` 时摘要多一个 `"insecureTls":true`，弱化过的测量因此不会被误读成正常测量）                                       | `bash ops/deploy/probe-loop.sh <url> <duration> <interval>`         |
...
>
> - **已完成并有实测支撑的**：`docker compose config --quiet` 在基础与生产两种 profile 下**退出码均为 0**（即这些文件确实能被真实的 compose CLI 解析，而不是"看起来像 YAML"）；`bash -n` 覆盖 `ops/` 下 **13 个** shell 脚本、**0 失败**；部署脚本桩化自测 **93 项全过**，其负向对照 **7 项全过**；健康门与探测循环打过**真实 HTTP 服务器**。详见[部署与运维](#部署与运维)与[基线记录](docs/aegis/baseline/2026-09-22-containerization-and-delivery.md)。
> - **未实测的**：镜像体积、镜像内 `mysql2/promise` 能否解析、容器能否起来、`migrate` 在容器内是否幂等、`nginx -s reload`、两个槽同时运行、零停机探测、告警投递——**全部未测**。（`nginx -t` 不在这个列表里了：2026-09-23 已在真机上跑过，并抓到两个会让生产栈起不来的 bug。）
...
**源码挂载刻意不给 `web`。** 运行时阶段是 Next.js 的 `standalone` 产物，镜像跑的是 `node web/server.js`，服务的是构建期内嵌的 `.next`——镜像里既没有 watcher 也没有构建步骤，挂源码**不会改变服务内容**；更糟的是服务的 bundle 来自镜像而可见的源码来自工作区，两者一有未提交改动就不一致，排查成本高于收益。**把"看起来在做热更新、其实没有"的配置写进仓库，比不写更坏**——它会让下一个人以为改了源码就生效。UI 的真实回路是宿主机 `pnpm dev`，或者先在宿主机上构建镜像再起容器：`docker build -f web/Dockerfile -t voting-web:dev .` 然后 `docker compose up -d web`（compose 文件里**没有** `build:` 段，所以 `docker compose up -d --build web` 什么都不会构建，只会去拉一个本地不存在的 `voting-web:dev`）；`migrate` / `indexer` 不同，它们用 `tsx` 直接执行 TypeScript，挂载确实立即生效。

### 生产发布

```bash
# 第一次起栈之前，必须先渲染一次 nginx 的上游文件。nginx.conf 用**精确路径**
# include upstream.conf，文件不存在时 nginx 直接拒绝启动，而这个文件由渲染脚本产生。
# 渲染在一次性容器里校验配置，所以它不需要 nginx 已经在跑：
...
**自签证书还要 `INSECURE_TLS=1`。** 观察窗用宿主机的 `curl` 打**公网**地址，证书不被信任时每一次请求都过不了校验，于是部署会回滚一个其实正常的版本——而且是在流量已经切过去之后。这个开关的效果与 `ops/blackbox/blackbox.yml` 里 `http_2xx_insecure` 模块相同：只证明"能连通"，不再证明"对面是谁"。它会写进探测摘要的 `"insecureTls":true`，让弱化过的测量不会被误读成正常测量。装上受信任的证书后同一个改动里去掉它。

**镜像从来不在服务器上构建。** 上传的包里没有应用源码，compose 文件里也没有 `build:` 段，`docker compose build` 在那里只会打印 `No services to build`。
...
**发布只能用 `ops/deploy/deploy.sh`，不能用 `up -d` 代替。** `up -d` 只是把新镜像的容器起起来：不跑迁移、不做健康门、不改上游文件，nginx 因此继续把流量发给旧槽——看起来成功了，而线上还是旧版本。真正的顺序写在 `ops/deploy/deploy.sh` 里，且**这个顺序是本设计最重要的一条**：
...
| `bash -n` 全部 shell 脚本                                            | **已实测**：`ops/` 下 **13 个**脚本（12 个 `.sh` + `ufw.rules`）、**0 失败**                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 部署脚本的顺序性质（桩化）                                           | **已实测**：`ops/deploy/selftest.sh` **93 项全过**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 上面那 93 项**会失败**                                               | **已实测**：`ops/deploy/selftest-controls.py` **7 项全过**——4 组注入故障各被抓住，改回后再次通过                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 健康门 / 探测循环打**真实 HTTP 服务器**                              | **已实测**：200→退出 0；死端口→退出 1（`status 000`）；404→退出 1（`status 404`）；预算精度 1s→**1.4s**、3s→**3.4s**、5s→**5.2s**                                                                                                                                                                                                                                                                                                                                                                                                        |
| 探测循环计数                                                         | **已实测**：全成功 `{"total":5,"succeeded":5,"failed":0,"maxConsecutiveFailures":0}` 退出 0；全失败 `{"total":3,"failed":3,"maxConsecutiveFailures":3,"firstFailureAtSeconds":0}` 退出 1；`INSECURE_TLS=1` 时多一个 `"insecureTls":true`，不设时不出现（负向对照在 `selftest.sh` 的 case 12 里）                                                                                                                                                                                                                                         |
...
| **`nginx -t` 与 `nginx -s reload`**                                  | **部分实测**：`nginx -t` 已在**真实服务器**上执行（2026-09-23 第一次真机渲染），`nginx -s reload` 仍未执行——nginx 至今没有启动过，graceful 行为未观察。第一次真机渲染**抓到两个 bug，两个都足以让生产栈起不来**：`nginx.conf` 里 `access_log ... main;` 写在 `log_format main` **之前**（nginx 解析时就查这个名字 → `[emerg] unknown log format "main"`），以及全新主机上 `nginx -t` 解析 `upstream` 块时 `web-blue` 还没有容器可解析（`[emerg] host not found in upstream`）。两处都已修，且渲染脚本不再把 nginx 的报错丢进 `/dev/null` |
| **两个槽同时运行**                                                   | **未实测**：从未同时启动过                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| **UI 在容器里的开发回路**                                            | **未验证**：运行时是 standalone 产物，挂源码不会热更新；真实回路是宿主机 `pnpm dev`，或 `docker build -f web/Dockerfile -t voting-web:dev .` 之后 `docker compose up -d web`（compose 文件里没有 `build:` 段）                                                                                                                                                                                                                                                                                                                           |
...
- **容器化与 CI/CD 的运行时证据仍然为零。** 镜像从未构建、容器从未启动、`compose up` 从未跑过；`nginx -s reload` 从未执行（`nginx -t` 已在真机上执行过，并抓到两个会让生产栈起不来的 bug）；两个槽从未同时运行；零停机的实验组与对照组都没做；监控栈从未启动、告警从未真实触发；GitHub Actions 从未触发（仓库已推到 `github.com:Zwh-1/decentralized-voting-dapp`，但没打过 `v*` tag）；未推送镜像、未产出 Trivy/SBOM；**2026-09-23 起有一台服务器**（`106.14.239.194`，阿里云 1.6 GB，上面还跑着另一个项目），有**自签**证书（不受任何客户端信任）、无域名、无 SMTP 凭据；**arm64 未覆盖**。已实测的是配置层面的可解析性、脚本的桩化顺序性质与负向对照、打过真实 HTTP 服务器的健康门与探测循环，以及**真机上的第一次渲染**（`docker compose config` 与 `nginx -t`）——**能被工具解析 ≠ 能运行**。完整清单见[基线记录](docs/aegis/baseline/2026-09-22-containerization-and-delivery.md)。
