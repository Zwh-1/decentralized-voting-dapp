...
| 6   | schema 迁移必须向后兼容            | 双槽发布期间新旧两版同时连一个库；破坏性变更必须拆成两次发布。见 [ADR-0044](docs/aegis/adr/ADR-0044-expand-contract-migrations-for-dual-slot-deploys.md) 与 [ops/runbook/migrations.md](ops/runbook/migrations.md) |

> **第 6 条的说明（2026-09-22）。** 它**不是新增的独立约束，而是第 3 条在发布维度的延伸**：第 3 条说事件与游标不能分开提交，因为两者共享同一份状态；第 6 条说新旧两版不能争抢同一份 schema，理由完全相同——**共享状态的两方不能被同时停机**。差别只在于第 3 条的时间尺度是一个事务，第 6 条是一个发布窗口。
>
> 具体含义：允许新增表、新增可空列或带默认值的列、新增索引与视图；**禁止在同一次发布里**删除或重命名旧版本仍在读写的列／表／视图，或给既有列加 `NOT NULL` 而无默认值。**破坏性变更拆两次发布**：第 N 次加新结构并回填（保留旧结构），第 N+1 次确认无旧版本运行后再删旧结构。
>
> 落地检查只有一行，发布前跑一次即可：
>
> ```bash
> git log --oneline <上一个 tag>..<本 tag> -- web/src/lib/db/schema.ts
> ```
>
> 输出非空即表示本次动了 schema，必须按上面两条走。**回滚不回滚 schema**——这是设计决定，理由是反向执行迁移会毁掉被放弃的那一版已经写入的数据，把一次坏发布变成数据丢失。因此若某次迁移不向后兼容，该次发布**不可回滚**，只能前滚修复。
