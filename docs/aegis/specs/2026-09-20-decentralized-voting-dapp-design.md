...
1. ~~**不做投票隐私**（commit-reveal / 零知识 / MACI）。~~
   **【已被 ADR-0031 部分取代】** 本条原意是「明票是唯一形态」。用户随后明确要求提供投票隐私，因此**「不做 commit-reveal」这一句已被取代**：commit-reveal 现在是一个**创建时可选**的机制（`PollConfig.commitReveal`），默认关闭。
   **仍然有效的是「不做零知识证明与 MACI」**：本 ADR 只实现隐私的一个受限子集——**投票过程中**选择不可见。揭示期之后选择照旧公开，参与事实（谁的票被计入）自始至终公开，地址与选票的关联也公开。因此它**不提供抗串谋、不提供匿名性、不能用于真实选举**。把这个边界写准比写宽更重要：commit-reveal 常被误当成「匿名投票」，而它并不是。
   决策依据见 ADR-0031（取代 ADR-0003）。
...
3. ~~**不做多轮次选举、提案制、委托投票**。~~
   ...
   **【「不做委托投票」这一句已被 ADR-0035 取代】** 本条曾写「委托会直接破坏一人一票」。这个判断需要更正：**它将"一人一票"误读为"一人一个投票动作"**。ADR-0034 已把该不变量重述为「每个合格主体的票权至多被计入一次」，而委托完全可以在守住这个上界的前提下成立——**谁行使**与**算谁的**分离，不等于同一份票权被计两次。
   现在委托是**创建时可选**的机制（`PollConfig.delegable`，默认关闭），边界由 ADR-0035 固定：**单层**（受委托人不得再委托）、**可撤回**（`to == address(0)`）、**委托人自己不得再投票**、**撤回时票权原地归还**。第四条正是"至多一次"上界的落地位置，`test_Delegate_DelegatingSubjectCannotAlsoVote` 钉住它。
   **仍然有效的是「提案制」**：那是治理流程，从未被要求。注意它与本条的标题并列，但两者命运不同——委托被机制化了，提案制没有。
   **一条必须保留的诚实说明**：委托**不破坏**每人一票的字面规则，但它**放大了个体影响力**——一个受委托人可以一次行使多人的票权。这不是漏洞，是委托的定义，但它意味着"结果由少数地址决定"在机制上变得更容易。该性质已在 README 的「设计取舍与已知局限」中披露（ADR-0035 的 Baseline Sync 动作）。
