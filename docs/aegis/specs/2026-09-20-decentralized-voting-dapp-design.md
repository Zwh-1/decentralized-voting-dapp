...
索引器单测 95 → **100**，全项目 159 → **164**；演练断言计数按实测更正为**只读 13、`--vote` 17、`--refund` 19**。见 ADR-0017。
...
| M-6e 浏览器  | 注入 EIP-1193 provider 后驱动真实 DOM：未白名单账户投票按钮**全部禁用**且给出理由；已白名单账户按钮**全部可点**，点击后到达"已确认"、卡片变为"你已投给该候选人"；`--refund` 场景退款按钮可点、点击后**从链上读回 `stakeOf=0`**、按钮随即禁用；三场景均断言 DOM 中**不存在**"提交中…"。`pnpm ui:drill` 退出码 0（只读 13、`--vote` 17、`--refund` 19 项断言，均在快照内实测），且三场景的浏览器控制台均无 error/warning——这条断言加上去的当次运行就抓到了 `/api/results` 的间歇 500，见 ADR-0017 |
...
| 测试总数     | Solidity 41 个 + TypeScript(viem) 23 个 + 索引器单测 100 个 = **164 个，全部通过**                                                                                                                                                                                                                                                                                                                                                                                                              |
