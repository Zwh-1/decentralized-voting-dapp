...
| 2026-09-20 | §4.3 M-3（前端）                   | 只做类型检查、生产构建、SSR 与 API 实测，未做浏览器端交互验证，投票按钮的可用性只检查阶段/连接/是否已投票 | 补上浏览器端演练 `pnpm ui:drill`：以链上 `isWhitelisted` 推导按钮可用性并给出理由，"进行态"改用 `receipt.isLoading`（`isPending` 对被禁用的查询恒为真，曾使按钮永远不可点）。见 ADR-0009 与规格校正 11                                            |
