...
| 一   | Task 1.6 本地 override                            | **完成**（加载未验）     | 新建 `docker-compose.override.yml`；base+override `config` 退出码 0，`web` 已发布 `127.0.0.1:3000`、`RPC_URL` 指向 `host.docker.internal:8545`、`extra_hosts` 渲染为 `host.docker.internal=host-gateway`、`restart: no`；migrate/indexer 各挂载源码 2 处。**另新写 `ops/ci/check-compose.py` 守护开发/生产隔离**（生产里 `host-gateway`/`3000`/`3307` 各出现 0 次），含检测器自测与真树注入验证（注入后退出 1 并指名泄漏）。**「容器真能连上宿主机链」及 UI 热更新路径需引擎**——运行时是 standalone 产物，挂源码不会热更新，已在 ops/README.md 写明 |
...
| 二   | Task 2.2 metrics 不暴露公网                       | **已写，加载未验**       | 三处叠加：生产 `web` 不发布端口（nginx 之外不可达）、nginx `location = /api/metrics { return 403; }`、Prometheus 只在内网抓 `web:3000`。**403 是否真生效、端口是否真不可达需引擎**                                                                                                                                                                                                                                                                                                                                                                  |
...
| 四   | Task 4.3 `deploy.sh` 双槽流程                     | **完成**（语义经桩验证） | 顺序：migrate → 起目标槽 → **直连目标槽**健康门 → 切上游 → 观察 → 记录。自测 **80 项全过**，负向对照 7 项全过                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 五   | Task 5.1 七条 ADR 落地                            | **完成**                 | 实际产出 **ADR-0045–0051**（计划原写 0040–0046；**执行时发现 0040–0044 已被并发 i18n 会话占用**，改为由 helper 分配编号，见 §14.13）。必备小节齐备、无重号；另对 **ADR-0015**（同一决策面，且它已否掉「上报 0」）与 **ADR-0006**（生产把进程内索引循环收窄为开发便利）追加修正记录。`check` 退出 1，但**154 条异常无一指向本工作流**：全部为既有遗留（0023–0029 缺短语、0033/0036–0039 未登记）或并发会话产物（0040–0044 未登记），按 anti-entropy-governance 需用户确认后才动                                                                      |
| 五   | Task 5.2 基线记录与简历材料                       | **完成**                 | 基线记录（按 §13.1 的 11 项；另列已实测 A-1…A-20 与未实测 U-1…U-44）、证据索引（18 张截图**全部标注待采集**并附采集命令）、README 新增部署与运维章节（+158/−2）。**镜像体积 / 流水线耗时 / 零停机失败数 / 回滚耗时全部写「未实测」且不给数字**；711 为引用 §14.5、已标注未重跑。核验时改正了5 处文件路径引用错误与一处不可复现的聚合数字，见 §14.13                                                                                                                                                                                                 |
...
> **编号已在执行时修正（2026-09-22）。** 本计划编写时（同日上午）核对到的现有 ADR 最大序号是 **0039**，因此原定新增 0040–0046。**实际执行时该前提已失效**：并发的另一个会话（i18n 工作流）已占用 **0040–0044**（`ADR-0040-locale-is-reader-chosen-…` 至 `ADR-0044-copy-extraction-boundaries`）。若按原编号创建，会覆盖他人的 ADR 或制造重号。
>
> 这条**只能由运行时决定，不能由计划冻结**：编号是共享可变量，任何写死它的计划都会在并发写者出现时失效。因此本批改为**由 `aegis-workspace.py new-adr` 在创建时自行分配**（helper 拥有编号权），实际产出为 **0045 起**。目录原清单不再逐条写死编号，只保留 slug 与主题。
>
> 顺带发现一处**语义**重叠（比编号冲突更重要）：原第 4 项「`lagBlocks` 不可读时序列 absent」与既有 **ADR-0015**（`lagBlocks` 只在滞后量成立时给出数字）**是同一个决策面**，且 ADR-0015 已经明确否掉了「无索引时上报 0」。按 `recording-architecture-decisions` 的要求「既有 ADR 已覆盖该决策面时应当修正而非另建同级文件」，该项**限定在新暴露面（`/api/metrics` 契约与告警）**并显式引用 ADR-0015，另对该条追加修正记录，避免两条 ADR 都声称拥有同一决策。

按 slug 与主题落地（编号由 helper 分配）：

- `one-image-carries-three-process-roles` —— 一个镜像承载 web / migrate / indexer 三个角色（被否：按角色拆多个 Dockerfile）
- `images-are-rebuilt-per-environment` —— 镜像按环境重建，`NEXT_PUBLIC_*` 构建期内联（被否：运行时注入 / 运行时配置端点）
- `observability-never-sits-in-the-startup-path` —— 监控栈不参与应用启动路径（被否：监控与监控对象同生共死）
- `unreadable-lag-is-absent-not-zero` —— 指标层不可读即为 absent，配 `absent()` 告警（被否：导出 0；**范围限定见上方说明，与 ADR-0015 同源**）
- `migrations-are-expand-contract-and-rollback-excludes-schema` —— expand-contract，回滚不含 schema（被否：可逆迁移）
- `deployments-pin-an-immutable-tag` —— 只用不可变 tag（被否：`latest`）
- `deployment-logic-lives-in-repo-scripts` —— 部署逻辑入库为脚本（被否：内联 YAML）
- 修正 `ADR-0015`（追加修正记录：同一条规则延伸到指标暴露面与告警层）
- 修正 `ADR-0006`（追加修正记录：生产拓扑下进程内索引循环被收窄为开发便利）
- 修改 `docs/aegis/INDEX.md`（由 helper 登记）
...
1. 逐条落地规格 §12 的 7 项，每条含：Context / Decision / Alternatives Considered（含**被否掉的理由**）/ Consequences / Compatibility Boundary / Retirement Impact / Baseline Sync / Evidence References：
   a. 一个镜像承载 web / migrate / indexer 三个角色（被否：按角色拆多个 Dockerfile）；
   b. 镜像按环境重建，而非运行时注入 `NEXT_PUBLIC_*`（被否：运行时配置端点）；
   c. 监控栈不参与应用启动路径（被否：监控与监控对象同生共死）；
   d. 指标层 `lagBlocks` 不可读时序列 absent，配 `absent()` 告警（被否：导出 0）——**范围限定在新暴露面，并修正 ADR-0015**；
   e. schema 迁移采用 expand-contract，回滚不含 schema（被否：可逆迁移）；
   f. 部署只使用不可变 tag（被否：用 `latest`）；
   g. 部署逻辑入库为脚本而非内联 YAML（被否：内联 YAML）。
...
### 14.6 Task 3.3 完成记录与四处修正（2026-09-22）

Docker 未解除，Task 3.3 的脚本仍可执行——它的中止语义可以用桩化的 `docker` 与 `curl` 驱动。已执行完毕。

**新增文件**（`ops/deploy/`，均 `chmod +x`）

| 文件                 | 作用                                                                                                            |
| -------------------- | --------------------------------------------------------------------------------------------------------------- |
| `lib.sh`             | 共用：`log` / `die` / `require_env` / `read_state` / `write_state` / `append_deploy_log` / `escape_label_value` |
| `health-gate.sh`     | 轮询端点直到 200 或预算耗尽；`0` 健康、`1` 超预算、`2` 参数错                                                   |
| `deploy.sh`          | `pull` → `migrate` → `up` → 健康门 → 记录状态与版本                                                             |
| `rollback.sh`        | 按 `previous-tag` 重新部署；**故意不跑迁移**                                                                    |
| `publish-version.sh` | 把当前 tag/slot 写成 node-exporter textfile 指标                                                                |
| `selftest.sh`        | 32 项检查，桩化 `docker`/`curl`                                                                                 |
...
| ---------------------------------------- | --------------------------------------------------------------------------- |
| `bash -n` 六个脚本                       | 全部通过                                                                    |
| `selftest.sh`                            | **32 通过 / 0 失败**，退出码 0                                              |
| 健康门对**真实服务**（`next` 起在 3200） | 退出码 0，日志 `passed: HTTP 200 on attempt 1 after 0s`                     |
| 健康门对**无服务端口**                   | 退出码 1，日志 `failed: last status 000 after 2 attempt(s) ... (budget 3s)` |
| 健康门**无参数**                         | 退出码 2                                                                    |
| `pnpm format:check`                      | 通过（prettier 无 shell 解析器，脚本不进该门禁）                            |

`selftest.sh` 覆盖的中止语义：migrate 失败→**从未 `up`、从未 `stop`、active-tag 未变、未发布版本**；健康门失败→`up` 过、`stop` 过、state 未被写成成功；成功→`active-tag`/`previous-tag`/`active-slot` 正确且 textfile 指标已写；回滚→**未调用 `run --rm migrate`**、且输出明说 schema 未回滚。

**四处修正**

1. **「健康门失败保持旧版本运行」在单槽下做不到**（Task 3.3 原 Steps）。启动新版本就已经替换了旧的，所以脚本改为：停掉不健康的服务、失败退出，恢复手段是 `rollback.sh`。计划原文那句只在双槽（批四）成立，已按事实改写而不是留一句做不到的承诺。脚本的执行序列已按「服务名参数化 + slot 概念」写好，批四的改动因此只落在 compose 文件与 upstream 翻转上。
2. **`sh -n` 用错了**（Task 3.3 原 Verification）。脚本是 bash 且用了 `set -o pipefail`，`sh -n` 会漏掉这一类。改为 `bash -n`。
3. **`shellcheck` 无法执行**。本机未安装（PowerShell 引号问题一度给出过一次假阳性，复核后确认未安装）。`shellcheck ops/deploy/*.sh` 是计划要求的验证项，**现记为未执行**。跟进方案：装进 CI（Task 3.1 的 `ci.yml` 加一个 shellcheck job），这样它同时成为门禁而不只是本地一次性检查。
4. **多出两个计划没写的产物，理由是它们替代了更差的做法**：
   - `publish-version.sh` 属 Task 2.7，但 `deploy.sh` 最后一步依赖它。没有给它加「文件不存在就跳过」的兜底——兜底会让部署看起来成功却什么都没记录，而「部署成功但旧版本还在服务」正是这套流水线要抓的失败。所以直接把它写完。
   - `selftest.sh` 不在计划里。加它是因为中止语义靠人工演练发现不了，而桩化驱动让这些语义**不需要 Docker 就能复现**。它不代表镜像可用（那是批一验收的事），只证明脚本在该停的地方停住了。
   - 另新增 `active-tag` 状态文件：计划的 `{active-slot, previous-tag}` 不足以让单槽回滚知道「回滚到哪个 tag」。

**健康门预算精度**：起初每次 `curl` 的 `--max-time 5` 不受预算约束，`budget 3s` 实测跑 6s——一个会悄悄超时的「预算」不算预算。改为单次尝试与尝试间隔都按剩余预算裁剪。改后实测：`budget 1s` → 1.3s，`budget 3s` → 4.6s。残余来自 1s 下限与 Windows 连关闭端口的延迟；**Linux 上的精确数值未验证**，不在本记录中声称。

### 14.7 standalone 入口实测：批一阻塞的机制级证据，兼 Task 2.1 证据补强（2026-09-22）

Task 2.1 的端到端证据当初是用 `next start` 取的。这轮发现 `next start` 在 `output: "standalone"` 下会打印 `⚠ "next start" does not work with "output: standalone" configuration` ——也就是说那份证据走的是**不受支持**的启动方式。于是补测了 Next 官方的 standalone 入口。

**结论：standalone 入口能正常服务**，Task 2.1 的证据因此成立且更硬：

- `PORT=3300 node .next/standalone/web/server.js` → `Ready`；
- `/api/health` → HTTP 200；`/api/metrics` → 30 行，含 `voting_index_lag_blocks 0`。

**但它「能用」的原因正是批一的阻塞**。`mysql2` 解析到的是：

```
D:\桌面\实习项目\decentralized-voting-dapp\node_modules\.pnpm\mysql2@3.24.4_...\node_modules\mysql2\promise.js
```

即**仓库根目录**的 `.pnpm`。`standalone/node_modules/` 下只有 `.pnpm`，没有 `mysql2` 顶层链接，所以 Node 沿目录向上逃逸：`standalone/web/node_modules` → `standalone/node_modules`（无链接）→ `.next/node_modules` → `web/node_modules` → **仓库根 `node_modules`**（命中）。
...
2. **我用 PowerShell 的 `-replace` 批量改 `deploy.sh`，把文件改坏了。** 替换串里含 `$0`，而 .NET 正则里 `$0` 表示「整个匹配」，于是原行被拼进了自己的替换结果，`deploy.sh` 第 51 行变成了 `usage "[ -n "$tag" ] || die "usage: $0 <tag>" <tag>"`。这正好撞在仓库已有的纪律上——**源文件只用文件工具改，不用 shell 重写**（这个仓库之前就因为 shell 重定向吃过 GBK 二次编码的亏）。已用 `edit` 修回，并补跑了 `bash -n` 与自测。
   ...
   同一批还有**第三次**伤害，而且我一开始误判成没事：那次 `Set-Content -Encoding utf8` 给 `deploy.sh` 加了 BOM，并把第 23、35 行注释里的破折号重新编码成乱码；`rollback.sh` 也被同样碰过（已还原，那次替换本身是空操作）。我当时的检查是「文件能不能按 UTF-8 解码」，结论是能，于是写下「strict-utf8 ok」——**这个结论是错的**：乱码后的字节仍然是合法 UTF-8，能解码不等于内容没变。正确的检查是同时看 BOM 与 U+FFFD／典型乱码字符；按这个标准重扫 `ops/`，结果 0 个问题文件（commit `3f0be30`）。

   这一条值得留在记录里，因为它示范了**验证方法本身选错会产出假证据**——比代码写错更值得警惕，而这个计划通篇都在防的正是这件事。两处注释现已改用 ASCII 的 `--` 表意，将来再被非 UTF-8 感知的工具处理也不会重演。
...
### 14.10 批三执行记录：CI/CD，以及一个只有靠机械检查才发现的并发缺口（2026-09-22）

**新增/修改**：`ci.yml`（加 `workflow_call` + 三个新 job）、`release.yml`、`rollback.yml`、`ops/ci/check-workflows.py`、`ops/ci/selftest.py`。

**已实测**

| 验证项                           | 结果                                                                   |
| -------------------------------- | ---------------------------------------------------------------------- |
| `ops/ci/check-workflows.py`      | 退出码 0（3 个 workflow 的不变量全部成立）                             |
| `ops/ci/selftest.py`             | **9 项全过**：8 个故意破坏各自被抓住，干净树通过                       |
| `ci.yml` job 数                  | 8 个（原 5 + shellcheck + observability-config + workflow-invariants） |
| `pnpm format:check`              | 通过（**格式化 workflow 后复跑不变量校验仍全过**）                     |
| `check-rules.py`                 | 退出码 0                                                               |
| `ops/deploy/selftest.sh`         | **41 项全过**                                                          |
| 编码扫描（`ops/` 与 `.github/`） | 0 个 BOM／乱码文件                                                     |

**未验证**：`actionlint` 本机不可用，workflow 的完整 schema 与表达式上下文未校验（自写检查器只覆盖仓库自己定的不变量，不覆盖 schema）；`gh workflow view` 无从执行（**没有 GitHub 仓库**）；`release.yml`/`rollback.yml` 从未真正跑过；`shellcheck` 未在本机跑过（已进 CI）。

**关键发现：`release.yml` 的 deploy job 缺并发锁——一个真的并发缺口**

设计里写明了「`concurrency: {group: deploy-production, cancel-in-progress: false}`」，`rollback.yml` 也用了这个组。但写 `release.yml` 时我把并发只放在了 **workflow 级**（`group: release-${{ github.ref }}`），**deploy job 上没有**。后果是：一次发布与一次回滚可以同时对着同一台主机执行，而两者都会改 active slot——**后完成的那个静默获胜**。

这个缺口不是读出来的，是检查器报出来的。更值得记的是修的过程里发现 `rollback.yml` 也有形式问题：它的锁在 **workflow 级**，而 `release.yml` 的在 **job 级**。workflow 级组的跨 workflow 竞争语义我不想依赖（这正是「靠一个微妙假设来保证互斥」的形状），所以**两边都改成 job 级**，形式完全对称——这样「它们是否争同一把锁」可以靠并排读两个文件来确认。

**为什么自写检查器而不是等 `actionlint`**

`actionlint` 校验 schema 与表达式，但它不可能知道这个仓库禁止 `ssh-keyscan`、禁止 `pull_request_target`、部署输入不得是可变 tag、发布与回滚必须共用锁。这四条每一条都是**合法 YAML、合法 workflow，但在这里是错的**。所以两者互补：CI 里两个都跑。

其中「不得使用可变 tag 部署」值得单独说：`latest` 会移动，于是「回滚到 `latest`」不是回滚到任何一个特定版本，而两次同名部署可以是两个不同的镜像。这是「测过的就是部署的」这条主张的**唯一**支撑。所以 `latest` 可以发布给人用，但**部署命令里出现它就报错**，这条由检查器强制。

**我自己写检查器时先犯的两个错**

1. **初版把 `rollback.yml` 的 workflow 级锁当成 job 级锁来查**，于是报了一个「不共用锁」的 finding。我第一反应是检查器写错了——**但去读文件才发现检查器是对的**：`rollback.yml` 确实写在 workflow 级。这正是检查器的价值：它逼我就一个我本来会靠「我记得我写了」来搪塞的细节去看实际字节。顺带这也说明为什么第一条反应不该是改检查器。
2. **禁止字符串的初版会命中自己的注释**。`release.yml` 里有一句注释在解释「为什么不用 `ssh-keyscan`」，而朴素的子串搜索会把它报成违规。我的初版 docstring 还专门写了「本检查不必区分解释与行为」——**那个理由站不住**：一个会对自己文档喊狼来了的检查，第一次被无视之后就等于不存在，而它看起来仍像是覆盖。改为先剥离整行注释再扫描，并只剥整行（行尾注释留着），这样假阴性仍然不可能——宁可假阳性可见可修，也不要假阴性。
...
| `ops/deploy/selftest-controls.py`（负向对照）                   | **7 项全过**：4 组注入故障各被抓住 + 3 项桩化环境自检（干净副本通过、移除故障后再次通过、副本运行后保持原状）                                                           |
...
### 14.12 批一收尾（Task 1.6）与发现的四个 CI 覆盖缺口（2026-09-22）

**新增**：`docker-compose.override.yml`、`ops/ci/check-compose.py`。**修改**：`ops/README.md`、`.github/workflows/ci.yml`。

**Task 1.6 已实测**：base+override 的 `docker compose config` 退出码 0；`web` 已发布 `127.0.0.1:3000`；`RPC_URL` 渲染为 `http://host.docker.internal:8545`；`extra_hosts` 渲染为 `host.docker.internal=host-gateway`（注意 compose 用 `=` 而非 `:`，我第一次的断言就是因为按 `:` 匹配而误报缺失）；`restart: 'no'`；`migrate`/`indexer` 各挂载源码 2 处。

**一处刻意偏离计划。** 计划 Task 1.6 第 1 步要求给 `web` 挂载 `./web/src` 以「热更新」。这条**做不到，且做了有害**：运行时阶段是 Next.js 的 `standalone` 产物，镜像跑的是 `node web/server.js`，服务的是构建期内联的 `.next`——镜像里既没有 watcher 也没有构建步骤，挂源码不会改变服务内容；更糟的是，服务的 bundle 来自镜像而可见的源码来自工作区，两者一有未提交改动就不一致，排查成本高于收益。因此 override **只给 `migrate`/`indexer` 挂源码**（这两个角色用 `tsx` 直接执行 TypeScript，挂载确实立即生效），并在 `ops/README.md` 写明 UI 的真实开发回路是宿主机 `pnpm dev` 或 `docker compose up -d --build web`。**把「看起来在做热更新、其实没有」的配置写进仓库，比不写更坏**——它会让下一个人以为改了源码就生效。

**发现并修掉的四个 CI 覆盖缺口**（都不是新写错，而是「写了证据但没人跑」）：

1. **`ops/nginx/render-upstream.sh` 从未被 shellcheck 覆盖。** 上一批的 shellcheck 命令只列了 `ops/deploy/*.sh ops/indexer/*.sh ops/alertmanager/*.sh`——批四新增的 nginx 目录不在其中。**通配符悄悄少匹配一个目录，job 照样报成功。** 现已显式列出每个目录，并新增一条 `bash -n` 遍历 `find ops -name '*.sh'`，使**将来新增目录也不会被遗漏**（遍历而非列举，是对这类遗漏的通用修法）。
2. **`ops/deploy/selftest.sh` 的 80 项断言从未在 CI 里跑过。** 这些断言守护的正是部署顺序——migrate 先于起槽、健康门先于切流量、旧槽留着当回滚目标。**而这三条在外部完全观察不到**：一个把流量切早了的部署会报告成功。把它们留在没人跑的脚本里，就等于把门禁降级成文档。
3. **`ops/deploy/selftest-controls.py` 的负向对照同样没跑。** 只跑自测而不跑对照，等于只声称「测试通过」而从不证明「测试会失败」。
4. **新写的 `ops/ci/check-compose.py` 没有入口。** 一个没人执行的检查器和不存在没有区别。

第 2 条与第 3 条合起来是本轮最值得记的一点：**上一批我花了很大力气证明「这 80 项会失败」，却从没让它进过 CI。** 证据只有在被自动执行时才是门禁，否则它只是我这一次的手工记录，下次改动照样能悄悄破坏它。

**`check-compose.py` 的内容与负向对照。** 它守护开发/生产隔离：生产配置里 `host-gateway`/端口 `3000`/`3307` 必须各出现 **0 次**，生产服务必须恰为 `{mysql, migrate, web-blue, web-green, nginx, indexer}`（且单槽 `web` 必须已被 profile 停用）。失败形态全部是静默的——发布的 3000 端口绕过 nginx（连带绕过拒绝 `/api/metrics` 的那条规则），而 `host.docker.internal` 的 `RPC_URL` 在云主机上不存在，表现为「页面能渲染、投票数为 0」。检查器含两部分：真实 `compose config` 的断言，加上一个**检测器自测**（把合成样本喂给检测函数，确认它会对 4 种泄漏报警、对 2 种正常配置不报）。此外做了**真树注入验证**：往 `docker-compose.prod.yml` 的 `web-blue` 注入 `extra_hosts`，检查器退出 1 并指名「production references the host gateway」。第一次注入尝试是往 override 里加——**那是无效的**（生产显式命名文件，根本不加载 override），且那次追加还产生了非法 YAML，使 `compose config` 直接失败；**那样得到的「失败」是因为 YAML 错，不是因为它发现了泄漏**，属于「为错误的原因而通过」。已改用注入生产文件本身重做。

**未实测**：容器是否真能通过 `host.docker.internal` 连到宿主机 `hardhat node`（Task 1.6 的验收命令要求 `chainId` 为 31337 且 `chainHead` 非 null）——需引擎。

**另一处环境干扰**：`.github/workflows/` 下的文件在工作区反复变成 CRLF，`release.yml`/`rollback.yml` 因此常驻 `git status` 的「已修改」，而 `git diff --ignore-cr-at-eol` 显示**内容差异为零**。索引里始终是 LF（`.gitattributes` 的 `eol=lf` 在 `add` 时归一化），所以提交内容不受影响，纯粹是工作区噪音。成因仍是并发会话的 `git checkout`。**这次我用「忽略行尾的 diff 是否为空」来区分「真实改动」与「行尾噪音」，而不是看到 `M` 就以为有人改了文件**——这个判据值得固定下来。

**CI job 数从 8 变 9**：`shellcheck` 改名为 `deploy-scripts`（现在真正包含 shellcheck 全目录 + `bash -n` 遍历 + 自测 + 负向对照），并新增 `compose-isolation`。`check-workflows.py` 与 `selftest.py` 在改名后复跑仍为退出码 0 与 9/0。

### 14.13 批五：七条 ADR、效果基线与简历材料（2026-09-22）

**新增**：`docs/aegis/adr/ADR-0045`…`ADR-0051`（7 条）、`docs/aegis/baseline/2026-09-22-containerization-and-delivery.md`、`docs/aegis/work/2026-09-22-delivery-evidence/90-evidence.md`、`ops/server/{daemon.json,ufw.rules,secrets.md}`。
**修改**：`ADR-0006`（追加修正记录）、`ADR-0015`（追加修正记录）、`docs/aegis/INDEX.md`、`README.md`（+158/−2）、`ops/deploy/selftest-controls.py`（重构）。

**已实测**：`pnpm format:check` 退出码 0；七条 ADR 的必备小节（`Status:` / Context / Decision / Alternatives Considered / Consequences / Compatibility Boundary / Retirement Impact / Baseline Sync / Evidence References）全部齐备；**编号无重号**（总计 51 条）；所有新文件 CRLF=0；`check-compose.py` 退出码 0；`selftest-controls.py` **7 项全过**；`check-rules.py` 退出码 0；`bash -n` 覆盖 `ops/` 下 10 个脚本 0 失败。

**`aegis-workspace.py check` 仍退出 1，但 154 条异常里没有一条指向本工作流的产物**（脚本按文件名过滤确认）。它们全部是既有遗留或并发会话的：

| 归属     | 内容                                     | 处置                                                        |
| -------- | ---------------------------------------- | ----------------------------------------------------------- |
| 既有     | `ADR-0023`–`0029` 缺标准短语段           | **不动**——非本计划范围                                      |
| 既有     | `ADR-0033`、`0036`–`0039` 未登记进 INDEX | **不动**，同上（计划 Task 5.1 第 4 步已写明需先经用户确认） |
| 并发会话 | `ADR-0040`–`0044`（i18n）未登记进 INDEX  | **不动**——属他人产物                                        |

这三类"修补他人产物"的动作，`anti-entropy-governance` 明确要求先取得显式确认，因此本批只登记了本工作流自己的两份新文档（baseline 与 evidence），已由 helper 成功登记。

**编号冲突（本批最重要的治理发现）**。计划编写时核对到的最大 ADR 序号是 **0039**，据此把本批编号写成 0040–0046。**执行时该前提已失效**：并发的 i18n 工作流已占用 **0040–0044**。若照原编号创建，会覆盖他人文件或制造重号。已改为**由 helper 在创建时自行分配**（helper 拥有编号权），实际产出 0045–0051。**教训：编号是共享可变量，任何把它写死在计划里的做法，都会在并发写者出现时失效。**

**一处语义重叠，比编号冲突更值得记**。原第 4 项「`lagBlocks` 不可读时序列 absent」与既有 **ADR-0015**（`lagBlocks` 只在滞后量成立时给出数字）**是同一个决策面**——而且 ADR-0015 已经**明确否掉了「无索引时上报 0」**这一被否方案。若另建一条同级 ADR，就会出现两条 ADR 都声称拥有"不可读时是什么"这一个决策面。按 `recording-architecture-decisions` 的要求（既有 ADR 已覆盖该面时应**修正**而非另建），处理方式改为：新 ADR 的范围**限定在新暴露面（`/api/metrics` 的契约与告警）**、显式引用 ADR-0015 说明同源关系，并对 ADR-0015 追加修正记录。同理对 **ADR-0006** 追加修正记录——它决定索引器由 `instrumentation.ts` 的后台循环驱动，而本阶段在生产把该循环收窄为开发便利（`INDEXER_ENABLED=false`，索引器成为独立容器），这是对既有决策面的收窄，属于修正而非新增。

**修正了两处证据错误（都由独立核验发现，不是自查）**：

1. **5 处引用的文件路径错了。** 基线记录原写「`README.md:91`、`:94`、`:95`、`:101`、`:105`」五处占位符命中。核验发现：**行号完全正确，文件错了**——这些占位符在 `ops/server/README.md`，而仓库根的 `README.md` 里一个都没有（`grep` 那五个变量名，零命中）。写的时候漏了目录前缀。**这个错误特别值得记，因为它看起来完全正常**：它指向一个真实存在的文件，行号也落在范围内。同仓库里有两个都叫 `README.md` 的文件，引用必须带路径。已改正并补上同文件内此前漏掉的 2 处（第 144、145 行）。
2. **聚合数字不可复现，已改为逐条枚举。** 原文给「228 个文件、命中 8 处」。用另一套排除集与模式复扫得到 212 文件 / 34 处——**换口径数字就变**，而原先没写出确切命令。已改为逐条枚举（11 行，含文件与行号），并说明聚合总数只在同一口径下才有意义。**构成证据的是逐条核对表，不是总数。**

同时把 `selftest-controls.py` 的项数从 6 更正为 7（见 §14.14），README 与基线里共 3 处引用已同步。

**未实测**（与基线 U-1…U-44 一致，此处只列与批五直接相关的）：`pnpm test` / `pnpm build:web` 本轮**未重跑**，因此 README 里的「711 passed」是**引用**计划 §14.5 的数字，已在 README 与基线两处标注"未重跑、也未与上文 177+327=504 核对"；镜像体积、流水线耗时、零停机失败数、回滚耗时**全部写「未实测」且不给任何数字**。

**一处安全事项（由子代理发现，此处留档）**：`contracts/.env` 内有明文真实 `SEPOLIA_PRIVATE_KEY`。它被 `.gitignore` 排除、**未入库**，但它确实在这台机器的磁盘上。同一台机器上有并发会话在跑全仓 prettier 与 git 操作，因此这条值得提醒使用者：这是一个真实测试网私钥。

### 14.14 一次真实事故：负向对照污染了被它测试的文件（2026-09-22）

**这是本阶段最值得留下的一次事故，因为它不是"测试写错了"，而是"测试工具本身有能力破坏被测对象"。**

`ops/deploy/selftest-controls.py` 为了验证「自测会失败」，需要让 `deploy.sh` 真的出错。初版的做法是：读出 `deploy.sh` 原文 → 写入一个注入故障的版本 → 跑自测 → 把原文写回去。这个做法在本会话里**两次**把注入的故障留在了工作区，`git status` 上表现为一次无法解释的、对已提交部署脚本的修改。第一次是「把 migrate 挪到起槽之后」，第二次是「让健康门失败仍记录成功」。

排查中又暴露出更关键的第二层问题：**"备份 + 恢复"这个设计依赖一个无法保证的前提——文件在我读它之前是干净的。** 当工作区里已经有一个残留故障时，脚本读到的"原文"就是被污染的内容，于是"恢复"忠实地恢复了污染，而结尾那句逐字节比对**拿同一个被污染的基线去比，报告了成功**。也就是说：**一个拿被测对象自身当基准的校验，不是校验。** 当时的实际输出正是"逐字节相同 ✓"加"恢复后的脚本不通过 ✗"——两句自相矛盾的话同时出现，这个矛盾就是线索。

**修法是消除前提，而不是加强守卫**：改为把 `ops/deploy` 与 `ops/nginx` 复制到临时目录，**在副本上注入故障**。`selftest.sh` 通过 `$here` 解析同级脚本，所以副本无需改动即可运行。这样"污染工作区"在结构上不可能发生，而不是"加了 try/finally 所以不太会发生"。同时新增一条断言：**运行结束后副本必须保持原状**——这样将来若有人把原地改写改回来，这条会失败。

顺带修掉同源的第二个毛病：原先用文本模式 `write_text` 恢复，在 Windows 上会把 `\n` 转成 `\r\n`——**这也是 `deploy.sh` 反复变回 CRLF 的原因之一**。现在全程走 `read_bytes`/`write_bytes`，不经过换行转换。

**可复用的判据**：任何"为了测试而修改被测文件"的工具，都应该**修改副本**而不是"修改后再还原"。还原是状态机，副本是隔离；前者会失败，后者不会。
