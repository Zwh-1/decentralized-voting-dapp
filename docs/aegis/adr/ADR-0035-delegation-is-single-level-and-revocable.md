...
**一、委托是单层的：受委托人不得再委托。拒绝发生在两侧。**

原决定只说了右侧：被指定的受委托人如果**自己**已经委托出去了，就拒绝。检查"某人是否有委托人"是错的，那会禁止多个主体指定同一个受委托人，而这恰恰是委托机制存在的意义（有负向对照测试 `test_Delegate_ManySubjectsMayNameTheSameDelegate` 钉住这一点）。

**但只有右侧不够，左侧同样必须拒绝**：一个**正在持有他人委托**的地址不得再把自己的权限转出去（`DelegateCannotDelegate`）。没有这一条时，A 委托给 B、B 再委托给 C，A 的权重就会被搁浅——B 的权限移到了 C，而 A 的增量仍记在 B 名下，同时 B 因为已经委托而按下面的规则报告 `controlledPowerOf == 0`。A 的权重于是不落在任何地址的账上。

这一条是被 `PollPowerConservation.t.sol` 的守恒属性找出来的，失败时的差额恰好等于一个主体的权重。撤回（`to == address(0)`）豁免此检查——它正是解开这种局面的方式，拦住它就等于让搁浅状态变成永久的。

**二、委托人在投票期可以随时指定或撤回委托，但已投票者必须先撤票。而受委托人一旦投票，委托不可收回。**
...
**撤回还有第三个约束**：受委托人一旦投票，它的票已经带着委托人的权重进入计票，此时收回权限（`DelegateHasVoted`）会让这份权重同时出现在已投出的票和委托人自己的账上。这与 `DelegatorHasVoted` 是同一条规则的两端——**一个主体的权限在一场投票中只移动一次**。

对称地，**已经投过票的地址不能再接受新的委托**（同样报 `DelegateHasVoted`）。它的票权已在 `votingPowerOf` 上定稿，而 `controlledPowerOf` 对已投票地址读的正是这个值；此后加入的委托只增加 `_delegatedSurplus`，而那个 view 已不再读它，于是新委托的权重计为空。

这两条也都是守恒属性找到的，且各有独立的负向对照：分别禁用后，属性在 round 41 与 round 22 以不同的差额失败。
...
**五、`controlledPowerOf` 永不报 0 给一个仍然控制着票权的地址；而一旦那部分权力已经进入计票，它也不再被允许变动。**

投票后 `vote` **不清空** `_delegatedSurplus`，因为撤回时必须靠它把票权还回去；`controlledPowerOf` 改为优先返回已计入的 `votingPowerOf`，因此不会重复上报。语义上：投票后"这个地址控制多少票权"仍是同一个数。

**但"撤回会把这份权力原地还回来"只在受委托人尚未投票时成立。** 那条路径的代价是把权限的移动次数从"任意次"收紧为"一次"：一旦票已投出，任何后续的委托或撤回都会被拒绝（见第二条）。这是刻意的取舍——允许事后变动就需要在计票里做补偿记账，而补偿记账正是守恒属性反复抓到出错的地方。
