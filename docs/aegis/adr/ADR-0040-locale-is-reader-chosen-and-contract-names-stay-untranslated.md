...
- 未完成的部分必须记明：组件文案的抽取当时只做了一部分——`ballot-reasons.ts`、塔标元数据、语言切换器、结果来源标签已完成；**`ballot-labels.ts` 的阶段/状态句与其余组件的文案仍是内联中文**。也就是说，英文界面当时是"部分翻译"，不是"完整英文"。这一点在 README 里也写明了，因为一个看起来完整的语言按钮配上中英混杂的页面比没有按钮更糟。
  - **后续（ADR-0044）**：这项已完成。抽取的最后阶段发现三类不在"字面量"里的中文——插值值（`chainName` / `phaseLabel` 的返回值）、测试钩子的属性值、以及 CSV 表头——并分别定下了边界。`src/` 下已无未接 `locale` 的文案函数调用点。
- `ui-drill` 继续在默认中文下运行，因此它对英文路径**没有覆盖**。英文路径目前由 `i18n.test.ts` 的目录完整性测试覆盖，没有被浏览器端到端验证过。
  - **后续（ADR-0044）**：这条限制**依然有效**。ADR-0044 追加了 `chrome-copy.test.ts` 与 `health-report.test.ts` 的英文块，但英文路径仍无浏览器端到端覆盖。
