...
3. 已经**实测**的那部分（`bash -n` 10/10、桩化自测 80 项、负向对照 7 项、`config` 退出码 0、真实 HTTP 健康门、`probe-loop` 计数）**同样是证据**，只是形式是命令输出而不是截图。它们的原始文本引用集中在 §3，**这些不是待采集**。
...
| `python ops/deploy/selftest-controls.py`           | **7 项全过**：干净副本通过 + 4 组注入故障各被抓住 + 移除故障后再次通过 + 副本运行后保持原状                                                                                  | 基线记录 §3 A-3      |
