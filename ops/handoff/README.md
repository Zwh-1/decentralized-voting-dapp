...
- **真实传输只在 2026-09-23 手工跑通过一次。** 首次端到端验证：对
  `alice@106.14.239.194` 用 `--dest /home/alice/voting` 上传成功
  （包 368640 字节 / 75 项，EXIT=0），服务器上解包成功，确认**无任何真实 `.env`
  被传走**。但这是**手工执行的一次**，不是自动化用例——`selftest.py` 仍然全部
  止步于 `--dry-run`，真正执行 `ssh` / `scp` 的那几行没有测试覆盖。
  回归仍然可能悄悄发生。
- **`--dest` 的默认值 `/srv/voting` 在多数服务器上不可用。** 实测那台机器
  `/srv` 属 root，普通用户 `mkdir -p /srv/voting` 直接 Permission denied，
  而脚本默认就用这个路径。要么给部署用户写权限，要么每次显式传 `--dest`。
  脚本**不会**事先探测这一点：它先打包再失败，白做一次传输准备。
- **脚本只接受密钥认证。** `ssh` / `scp` 都带 `-o BatchMode=yes`，因此**不提示
  输入密码**——这是刻意的（密码提示在自动化里表现为卡死而非报错），但也意味着
  一台只开了密码认证、没装公钥的服务器上，脚本永远返回 1。实测那台机器先是
  `Permission denied`，装上 `authorized_keys` 后立即通过。
- **`.bat` 与 `.sh` 的一致性只覆盖参数校验和凭据守卫。** 两者都实现同一套规则，
  但比对只做到退出码层面、且只用那些"不碰网络也不写文件"的参数组合。真正会分叉的
  地方（`git archive` 的打包口径、`ssh`/`scp` 的失败处理）是靠各自单独测的，
  没有交叉验证。
