@echo off
REM ---------------------------------------------------------------------------
REM Windows launcher: upload the deployment files to a server.
REM Double-clickable, and usable from PowerShell or cmd.
REM
REM   upload-to-server.cmd deploy@YOUR_SERVER_IP
REM   upload-to-server.cmd deploy@YOUR_SERVER_IP --dry-run
REM   upload-to-server.cmd deploy@YOUR_SERVER_IP --port 2222 --dest /srv/voting
REM
...
REM
REM The real logic lives in upload-to-server.sh. This does three things: find
REM Git's bash, switch to the repository root, and forward the arguments
REM unchanged. It deliberately does NOT carry its own usage text or argument
REM validation -- an earlier revision duplicated the usage text and the two
REM copies disagreed: the .cmd returned 0 on a usage error where the .sh
REM returned 2. Run it with no arguments to see the usage the .sh prints.
REM
