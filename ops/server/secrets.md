...
| `PUBLIC_HEALTH_URL`           | The address a user actually opens, e.g. `https://106.14.239.194:8443/api/health`. `deploy.sh` and `rollback.sh` observe it after moving traffic and treat a failure as a failed release. Passed through by `release.yml` and `rollback.yml`; unset expands to empty, and empty falls back to `https://localhost/api/health`, which cannot succeed on a host that serves on another port. |
| `INSECURE_TLS`                | `1` while the certificate is self-signed: curl cannot verify it, so the observation window would fail and roll back a working release. Same trade as the `http_2xx_insecure` module in `ops/blackbox/blackbox.yml`, and it is recorded in the probe summary as `"insecureTls":true`. Delete this Variable in the same change that installs a certificate clients trust.                  |
